  ...
  class TcpEchoClient : public Application
        {
          ...
          private:
                  /*
                  All method are called automatically by ns3 core ( regisered as callback )
                  */
                  /*
                  ...
                  */
                  ...
                  Ptr<Socket> m_socket;                                            // socket object [https://docs.oracle.com/javase/tutorial/networking/sockets/definition.html]
                  Address m_peer;                                                  // server address
                  uint32_t m_packetSize;                                           // packet side
                  uint32_t m_nPackets;                                             // number of packets to be sent
                  uint16_t m_peer_port;                                            // server application port
                  Ipv4Address m_local;                                             // client machine address
                  DataRate m_dataRate;                                             // client application datarate
                  EventId m_sendEvent;                                             // used to store the schdualed event for next send , so that it is cancelled when needed
                  bool m_running;                                                  // a flag to indicate if this  application is emmiting traffic 
                  double m_sendTime;                                               // the timestamp in which to start emmiting traffic
                  uint32_t m_packetsSent;                                          // number of packets send
                  uint32_t m_packetsRecieved;                                      // number of echo packets received
                  uint32_t m_bytesSent;                                            // the number of bytes sent
                  uint32_t m_bytesRecieved;                                        // the number of bytes received
                  double m_packetsTransmissionTime;                                // an accumilator used to compute average  received packet delay 
                  uint32_t m_maxRecievedPacketSize;                                // used to store the size of the biggest received packet
                  uint32_t m_minRecievedPacketSize;                                // used to store the size of the smallest received packet
                  SimpleHistogram<uint32_t> m_RecievedPacketsSizeHistogram;        // histogram for the size of received packets
                  SimpleHistogram<uint32_t> m_sentPacketsSizeHistogram;            // histogram for the size of sent packets
                  double m_delay;                                                  // the time interval after which to send a packet periodically following the given datarate
        ...
        */
                                  ...
                                  )
        {
          ...
          NS_LOG_UNCOND("m_delay : " << m_delay << "second");
        ...
        */
        void TcpEchoClient::StartApplication(void)
        {
          ...
          m_socket->Bind(local);
        #endif
          NS_LOG_UNCOND("TcpEchoClient::StartApplication At : "<<Simulator::Now().GetSeconds() );
        ...
        */
        void TcpEchoClient::ShowStats()
        {
          ...
          NS_LOG_UNCOND("client intended packets sent         : " << m_nPackets);
        // NS_LOG_UNCOND("client intended packet size          : " << m_packet Size);
          NS_LOG_UNCOND("client packets sent                  : " << m_packetsSent);
          NS_LOG_UNCOND("client packets received              : " << m_packetsRecieved);
          NS_LOG_UNCOND("client bytes sent                    : " << m_bytesSent);
          NS_LOG_UNCOND("client bytes received                : " << m_bytesRecieved);
          NS_LOG_UNCOND("client average received packet size  : " << double(m_bytesRecieved) / double(m_packetsRecieved));
          NS_LOG_UNCOND("client min received packet size      : " << m_minRecievedPacketSize);
          NS_LOG_UNCOND("client max received packet size      : " << m_maxRecievedPacketSize);
          NS_LOG_UNCOND("client packet total transmission time : " << double(m_packetsTransmissionTime));
          NS_LOG_UNCOND("client average received packet delay : " << double(m_packetsTransmissionTime) / double(m_packetsRecieved));
          NS_LOG_UNCOND("client first packet send time : " <<  this->m_firstPacketSendTime<<" second");
          NS_LOG_UNCOND("client last packet send time : " <<  this->m_lastPacketSendTime<<" second");
          NS_LOG_UNCOND("client average send time : " <<  m_average_send_time / m_packetsSent<<" second");
          NS_LOG_UNCOND("client average receive time : " <<  m_average_receive_time / m_packetsRecieved<<" second");
          ...
          NS_LOG_UNCOND("client average packet reception delay :"<<m_delay);
          ...
          NS_LOG_UNCOND("client last packet receive time : " << m_lastPacketReceiveTime<<"\n");
          // NS_LOG_UNCOND("client intended packet size          : " << m_packet Size);
        }
           /*
        ...
        */
        void TcpEchoClient::SendPacket(void)
        {
          ...
          else
          {
            ...
            NS_LOG_UNCOND("client close time : "<<Simulator::Now().GetSeconds() 	);
        ...
        */
        ...
        {
          NS_LOG_UNCOND("TcpEchoClient::ConnectionSucceeded  AT : " << ((double)Simulator::Now().GetSeconds()));
          
          Time tNext(Seconds( std::max(  0.0d   , this->m_sendTime - ((double)Simulator::Now().GetSeconds()))));
          // schedual next send for tsend
          NS_LOG_UNCOND("First client send at : " << tNext.GetSeconds() << " second");
        ...
        */
        void TcpEchoClient::HandleRead(Ptr<Socket> socket)
        {
          ...
          /*
            Like the the the "TcpEchoClient" calass, the "TcpEchoServer" is a class for defining the server logic in the network,
            which will be installed on the target host machines 
            ...
            It Listens on a given port until a client connects, then confirme the  TCP connection with the client.  Every time the server received a data packet,
            it either ignores it or sends an echo the echo can be of the same size as the received packet or can be of N times greater , with N = echo factor,
      ...
      class TcpEchoServer : public Application
      {
              public:
                ...
                */
                ...
                private:  
                            /*
                            All those methods are called automatically by ns3 core ( regisered as callback )
                            */
                          /**
                          ...
                          */
                          ...
                          uint16_t m_port;                                                  // listening port of the server
                          Ptr<Socket> m_socket;                                             // listening socket
                          Ipv4Address m_local;
                          uint32_t m_delay;                                                 // echo timedelay in seconds
                          uint32_t m_packetsSent;                                          // number of packets send
                          uint32_t m_packetsRecieved;                                      // number of echo packets received
                          uint32_t m_bytesSent;                                            // the number of bytes sent
                          uint32_t m_bytesRecieved;                                        // the number of bytes received
      ...
      void TcpEchoServer::HandleAccept(Ptr<Socket> s, const Address &from)
      {
        ...
        NS_LOG_UNCOND("Accept at : "<<Simulator::Now().GetSeconds());
      ...
      void TcpEchoServer::ShowStats()
      {
        ...
        NS_LOG_UNCOND("server packets sent                  : " << m_packetsSent);
        NS_LOG_UNCOND("server packets received              : " << m_packetsRecieved);
        NS_LOG_UNCOND("server bytes sent                    : " << m_bytesSent);
        NS_LOG_UNCOND("server bytes received                : " << m_bytesRecieved);
        NS_LOG_UNCOND("server average received packet size : " << double(m_bytesRecieved) / double(m_packetsRecieved));
        NS_LOG_UNCOND("server min received packet size     : " << m_minRecievedPacketSize);
        NS_LOG_UNCOND("server max received packet size     : " << m_maxRecievedPacketSize);
        NS_LOG_UNCOND("server packet total transmission time : " << double(m_packetsTransmissionTime));
        NS_LOG_UNCOND("server average packet reception delay : " << double(m_packetsTransmissionTime) / double(m_packetsRecieved));
        NS_LOG_UNCOND("server first packet send time : " <<  this->m_firstPacketSendTime<<" second");
        NS_LOG_UNCOND("server last packet send time : " <<  this->m_lastPacketSendTime<<" second");
        NS_LOG_UNCOND("server average send time : " <<  m_average_send_time / m_packetsSent<<" second");
        NS_LOG_UNCOND("server average receive time : " <<  m_average_receive_time / m_packetsRecieved<<" second");
      ...
      void TcpEchoServer::HandleRead(Ptr<Socket> socket)
      {
        ...
        // end of jitter
          ...
          if (m_echoFactor)
          {
            ...
            if (InetSocketAddress::IsMatchingType(from))
            {
              NS_LOG_UNCOND_ON_OFF("At time " << Simulator::Now().As(Time::S) << " server sent " << packet->GetSize() << " bytes to " << InetSocketAddress::ConvertFrom(from).GetIpv4() << " port " << InetSocketAddress::ConvertFrom(from).GetPort());
            }
            else if (Inet6SocketAddress::IsMatchingType(from))
            {
            ...
            {
              Time tNext(MilliSeconds(m_delay));
...
{
 NS_LOG_UNCOND_ON_OFF (Simulator::Now ().GetSeconds () << "\t" << newCwnd);
}*/
/*
...
void ClientIpV4L3SendOutgoingCallback()
{
  ...
  // NS_LOG_UNCOND("ClientIpV4L3SSendOutgoingCallback ");
}
...
void ClientIpV4L3TX()
{
  ...
  //  NS_LOG_UNCOND("ClientIpV4L3TX ");
}

void ServerIpV4L3SendOutgoingCallback()
{
  ServerIpV4L3SendOutGoingCount++;
  // NS_LOG_UNCOND("ServerIpV4L3SSendOutgoingCallback ");
}

void ServerIpV4L3TX()
{
  ServerIpV4L3STxCount++;
  // NS_LOG_UNCOND("ServerIpV4L3TX ");
}
int simulate(
    double error_rate,
...
{
  //std::string tr = "/mnt/c/Users/majdr/OneDrive/Desktop/test2.tr";
  // std::string xmlName = "/mnt/c/Users/majdr/OneDrive/Desktop/test1.xml";
  // std::string pPcap = "/mnt/c/Users/majdr/OneDrive/Desktop/point-to-pointcableCapture" ; 
...
// start network setup
  ...
  else
  {
    ...
    // set the data rate of the channel
//    p2p->SetChannelAttribute("DataRate", StringValue(channel_data_rate));
...
// end network setup
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter)
  {
    ...
    NS_LOG_UNCOND( "first time send by "<<t.sourceAddress <<"  : "<<iter->second.timeFirstTxPacket.GetSeconds() );
    NS_LOG_UNCOND( "last time send by "<<t.sourceAddress<< "  : "<<iter->second.timeLastTxPacket.GetSeconds() );
    NS_LOG_UNCOND( "first time received by "<<t.destinationAddress <<"  : "<<iter->second.timeFirstRxPacket.GetSeconds() );
    NS_LOG_UNCOND( "last time received by "<<t.destinationAddress<< "  : "<<iter->second.timeLastRxPacket.GetSeconds() );
  ...
  NS_LOG_UNCOND("Server Number of collisions : "<<ServerMacTxBackoffOccurCount);
  NS_LOG_UNCOND("Client Number of collisions : "<<ClientMacTxBackoffOccurCount);
...
int main(int argc, char *argv[])
{
  ...
  cmd.AddValue("error_rate", "error rate for netdevice error model", error_rate);
  cmd.AddValue("channel_data_rate", "channel_data_rate", channel_data_rate);
  cmd.AddValue("channel_delay", "channel_delay", channel_delay);
  cmd.AddValue("point_to_point_queue_size", "point_to_point_queue_size", point_to_point_queue_size);
  cmd.AddValue("server_echo_factor", "server_echo_factor", server_echo_factor);
  cmd.AddValue("server_echo_delay", "server_echo_delay", server_echo_delay);
  cmd.AddValue("client_data_rate", "client_data_rate", client_data_rate);
  cmd.AddValue("client_packets_count", "client_packets_count", client_packets_count);
  cmd.AddValue("client_packet_size", "client_packet_size", client_packet_size);
  cmd.AddValue("client_start_time", "client_start_time", client_start_time);
  cmd.AddValue("client_send_time", "client_send_time", client_send_time);
  cmd.AddValue("server_start_time", "server_start_time", server_start_time);
  cmd.AddValue("client_stop_time", "client_stop_time", client_stop_time);
  cmd.AddValue("server_stop_time", "server_stop_time", server_stop_time);
  cmd.AddValue("simulation_length", "simulation_length", simulation_length);
  cmd.AddValue("use_csma", "use csma channel ", use_csma);
  cmd.AddValue("new_reno_enabled", "new_reno_enabled", new_reno_enabled);
  cmd.Parse(argc, argv);
