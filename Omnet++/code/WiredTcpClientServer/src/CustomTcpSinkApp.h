 ...
 * on them.
 */
class INET_API CustomTcpSinkApp : public CustomTcpServerHostApp
{
  protected:
    long bytesRcvd = 0;
  ...
  public:
    ...
    friend class CustomTcpSinkAppThread;
};

class INET_API CustomTcpSinkAppThread : public TcpServerThreadBase
{
  protected:
    long bytesRcvd;
    CustomTcpSinkApp *sinkAppModule = nullptr;
    ...
    virtual void init(CustomTcpServerHostApp *hostmodule, TcpSocket *socket) override { TcpServerThreadBase::init(hostmodule, socket); sinkAppModule = check_and_cast<CustomTcpSinkApp *>(hostmod); }
};
