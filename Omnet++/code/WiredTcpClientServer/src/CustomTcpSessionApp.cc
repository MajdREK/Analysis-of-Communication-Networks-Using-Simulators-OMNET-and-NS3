            ...
            for (auto& region : regions) { // for each region do
                ...
                if (tag)
                {
                    ...
                    // calculate jitter
                     ...
                     m_PreviousPacketDelay =  delay.dbl(); // we save the current packet delay to be used for next time to calculate delay diffrence like on the top
                     // end of jitter
...
void CustomTcpSessionApp::finish()
{
    ...
    //auto eventsCount = cSimulation::getActiveSimulation()->getFES()->getLength();
    //EV<<"Events count : "<<eventsCount<<omnetpp::endl;
}
