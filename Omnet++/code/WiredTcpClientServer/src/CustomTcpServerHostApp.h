 ...
 */
class INET_API CustomTcpServerHostApp : public ApplicationBase, public TcpSocket::ICallback
{
  protected:
  ...
  public:
    virtual ~CustomTcpServerHostApp() {
        EV << "~CustomTcpServerHostApp" << omnetpp::endl;
...
/**
 * Abstract base class for server processes to be used with CustomTcpServerHostApp.
 * Subclasses need to be registered using the Register_Class() macro.
 *
 * @see CustomTcpServerHostApp
 */
class INET_API TcpServerThreadBase : public cSimpleModule, public TcpSocket::ICallback
{
  protected:
    CustomTcpServerHostApp *hostmod;
    TcpSocket *sock;    // ptr into socketMap managed by CustomTcpServerHostApp

    // internal: TcpSocket::ICallback methods
    virtual void socketDataArrived(TcpSocket *socket, Packet *msg, bool urgent) override {
        EV <<"TcpServerThreadBase::socketDataArrived"<<omnetpp::endl;


        simtime_t startTime, endTime;
        if (msg->isReceptionStart())
        {
        // gate was reprogrammed with setDeliverOnReceptionStart(true)
        startTime = msg->getArrivalTime(); // or: simTime();
        endTime = startTime + msg->getDuration();
        }
        else
        {
        // default case
        endTime = msg->getArrivalTime(); // or: simTime();
        startTime = endTime - msg->getDuration();
        }
        EV << "interval: " << startTime << ".." << endTime << "\n";
        EV << "duration: " <<endTime -  startTime  << "\n";
  ...
  public:
    ...
    // internal: called by CustomTcpServerHostApp after creating this module
    virtual void init(CustomTcpServerHostApp *hostmodule, TcpSocket *socket) { hostmod = hostmodule; sock = socket; }
     ...
     */
    virtual CustomTcpServerHostApp *getHostModule() { return hostmod; }
